...
function addMatrices(matrix1, matrix2){ 
    ...
    if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length){
        console.error("Error! Matrices are not of the same size.");
        return;        
    }
...
const multiplyMatrices = (matrix1, matrix2) => { 
    ...
    if (matrix1[0].length !== matrix2.length){ // check if the number of columns in the first matrix is equal to the number of rows in the second matrix
        console.error("Matrix Multiplication is not possible. Number of columns in first matrix does not match number of rows in second matrix.");
        return null;
    }
